...
void main() {
    ...
    vAlpha = smoothstep(0.5, 1., normalize(worldPosition.xyz).z) * .8 + .2;
    vUvs = aModelUvs;
...
let lastResizeCanvasCall = 0;
function resizeCanvasToDisplaySize(canvas) {
  const now = Date.now();
  if (now - lastResizeCanvasCall > 100) {
    // 10fps throttle
...
class InfiniteGridMenu {
  ...
  #updateProjectionMatrix(gl) {
    ...
    const height = this.SPHERE_RADIUS * 0.3;
...
export default function InfiniteMenu({ items = [] }) {
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined" && "requestIdleCallback" in window) {
      window.requestIdleCallback(runHeavyLogic);
    } else {
      setTimeout(runHeavyLogic, 0);
    }
